...
5. **Onda 5 — Acessibilidade e polimento**: ✅ foco visível consistente (`:focus-visible` global com `--ring` + offset). ✅ áreas de toque ≥ 44 px em `pointer: coarse` (mobile/tablet) sem inflar o desktop. ✅ utilitários `pb-safe` / `pt-safe` para `safe-area-inset`. ✅ classe `edit-mode-transition` (150 ms) para o toggle de Modo Edição. ✅ respeito a `prefers-reduced-motion`. ⏸️ modo compacto opcional e empty-states ilustrados: ficam para iteração futura (exige tocar muitas listas).
6. **Onda 6 — Polimento premium**: ✅ 6.1 tokens + tipografia (Plus Jakarta Sans display, clamp fluido), sombras (`--shadow-soft/hover/focus`), dark mode auditado, `ThemeToggle` no dashboard com persistência local. ✅ 6.2 microinterações CSS-only: tap-feedback global (scale 0.97 em :active), `.skeleton-shimmer`, `tab-indicator-in` (fade+slide-up via `[data-state="active"][role="tab"]`), toasts coloridos por tipo (Sonner `richColors` + barra lateral via CSS). ✅ 6.3 dashboard refinado: cabeçalho com data em uppercase tracked (0.12em) + título com gradient `from-foreground to-foreground/70` (clip-text), `CollapsibleCard` com chip colorido para o ícone (8×8, `bg-primary/10`, radius-sm), sombra `--shadow-soft` + lift para `--shadow-hover` no hover (180ms), chevron alinhado verticalmente ao novo chip. Próxima: 6.4 Semana da Visita (subtitles, sticky headers, compact mode <380px).
