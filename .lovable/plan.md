...
6. **Onda 6 — Polimento premium**: ✅ 6.1 tokens + tipografia + sombras + dark mode + ThemeToggle. ✅ 6.2 microinterações CSS-only (tap-feedback, skeleton-shimmer, tab-indicator-in, toasts coloridos). ✅ 6.3 dashboard refinado (header gradient, chip do CollapsibleCard, hover lift). ✅ 6.4 Semana da Visita: TabsList sticky com backdrop-blur (`.visit-week-tabs-sticky`, top:0 z:20), hint contextual por aba (`tabHintCampo/Meio/Fim/Pioneiros/Ancios` em pt/en/es) com `aria-live="polite"`, compact mode <380px (abas menores, padding reduzido). Próxima: 6.5 Listas otimizadas (react-window onde >50 itens) e 6.6 acessibilidade (focus rings, aria, contraste WCAG AA).
