...
6. **Onda 6 — Polimento premium**: ✅ 6.1 tokens + tipografia (Plus Jakarta Sans display, clamp fluido), sombras (`--shadow-soft/hover/focus`), dark mode auditado, `ThemeToggle` no dashboard com persistência local. ✅ 6.2 microinterações CSS-only: tap-feedback global (scale 0.97 em :active), `.skeleton-shimmer` (gradiente animado GPU-friendly), `tab-indicator-in` (fade+slide-up na aba ativa via `[data-state="active"][role="tab"]`), toasts coloridos por tipo (Sonner `richColors` + barra lateral + ícone tonalizado por `--success/--destructive/--warning/--primary`). Próxima: 6.3 dashboard refinado.
