...
6. **Onda 6 — Polimento premium**: ✅ 6.1 tokens + tipografia + sombras + dark mode + ThemeToggle. ✅ 6.2 microinterações CSS-only (tap-feedback, skeleton-shimmer, tab-indicator-in, toasts coloridos). ✅ 6.3 dashboard refinado (header gradient, chip do CollapsibleCard, hover lift). ✅ 6.4 Semana da Visita: sticky tabs com backdrop-blur, hint contextual com `aria-live`, compact mode <380px. ✅ 6.5 Listas otimizadas CSS-only (`content-visibility:auto` + `contain-intrinsic-size`). ✅ 6.6 Acessibilidade: anel de foco com halo (`--shadow-focus`), skip-to-content link (WCAG 2.4.1), `<main id="main-content" tabIndex={-1}>`, `aria-label` no menu hamburger + `aria-hidden` no ícone, contraste WCAG AA reforçado (`--muted-foreground` 0.5 → 0.45 no claro), chaves `a11y.*` em pt/en/es.
