...
function Page() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (!/^[A-Z0-9]{4,12}\*?$/.test(c)) { toast.error(t("guest.invalidCode")); return; }
    setBusy(true);
    ...
    <div className="min-h-screen bg-gradient-to-br from-primary via-primary to-primary-soft/40 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="text-center text-primary-foreground mb-6">
          ...
          <h1 className="text-2xl font-bold tracking-tight">{t("guest.loginTitle")}</h1>
          <p className="text-sm text-primary-foreground/80 mt-1">{t("guest.loginSubtitle")}</p>
        </div>
        <Card className="shadow-elevated border-0">
          <CardContent className="p-6">
            <div className="flex items-center gap-3 mb-5">
              ...
              <div>
                <h2 className="font-semibold text-lg leading-tight">{t("guest.readOnly")}</h2>
                <p className="text-xs text-muted-foreground">{t("guest.enterCode")}</p>
              </div>
            </div>
            <form onSubmit={submit} className="space-y-3">
              <div>
                <Label htmlFor="code">{t("guest.code")}</Label>
                <Input id="code" className="mt-1 font-mono uppercase tracking-widest" maxLength={13}
                  value={code} onChange={(e) => setCode(e.target.value.toUpperCase())} placeholder={t("guest.codePlaceholder")} />
              </div>
              <Button type="submit" className="w-full h-11" disabled={busy}>
                <Eye className="mr-2 h-4 w-4" /> {t("guest.access")}
              </Button>
            </form>
          </CardContent>
        </Card>
        <Link to="/" className="mt-4 w-full text-sm text-primary-foreground/90 hover:text-primary-foreground inline-flex items-center justify-center gap-2 py-2">
          <ArrowLeft className="h-4 w-4" /> {t("guest.backToLogin")}
        </Link>
      </div>
    </div>
  );
