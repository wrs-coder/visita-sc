...
export function RichNoteToolbar({
  ...
  // ===== Modo padrão (2 filas premium, scroll horizontal) =====
  ...
  const rowPopover = (children: ReactNode) => (
    <PopoverContent
      side="top"
      align="start"
      className="w-auto p-1 z-[120] flex flex-row items-center gap-1"
  ...
  // Ícone dinâmico do botão de alinhamento (reflete o estado atual).
  const alignIcon = isActive({ textAlign: "center" } as never) ? (
    <AlignCenter className="h-4 w-4" />
  ) : isActive({ textAlign: "right" } as never) ? (
    <AlignRight className="h-4 w-4" />
  ) : isActive({ textAlign: "justify" } as never) ? (
    <AlignJustify className="h-4 w-4" />
  ) : (
    <AlignLeft className="h-4 w-4" />
  );

  // Rótulo dinâmico do seletor de bloco.
  const blockValue = isActive("heading", { level: 2 })
    ? "h2"
  ...
  return (
    <div
      ...
      onMouseDown={(e) => e.preventDefault()}
    >
      {/* ===== Fila 1 — Formatação de texto ===== */}
      {row(
        <>
          {/* Seletor de bloco (¶ / H2 / H3 / Citação) */}
          <Select value={blockValue} onValueChange={onBlockChange}>
            <SelectTrigger
              className="h-8 w-[92px] px-2 text-xs shrink-0"
              onMouseDown={(e) => e.preventDefault()}
              aria-label={t("personalOutlines.editor.toolbar.normal")}
            ...
            <SelectContent className="z-[130]">
              <SelectItem value="p" className="text-xs">
                <span className="inline-flex items-center gap-2"><Pilcrow className="h-3.5 w-3.5" />{t("personalOutlines.editor.toolbar.normal")}</span>
              </SelectItem>
              <SelectItem value="h2" className="text-xs">
                <span className="inline-flex items-center gap-2"><Heading2 className="h-3.5 w-3.5" />{t("personalOutlines.editor.toolbar.title")}</span>
              </SelectItem>
              <SelectItem value="h3" className="text-xs">
                <span className="inline-flex items-center gap-2"><Heading3 className="h-3.5 w-3.5" />{t("personalOutlines.editor.toolbar.subtitle")}</span>
              </SelectItem>
              <SelectItem value="quote" className="text-xs">
                <span className="inline-flex items-center gap-2"><Quote className="h-3.5 w-3.5" />{t("personalOutlines.editor.toolbar.blockquote")}</span>
              </SelectItem>
            </SelectContent>
          </Select>
          ...
          <Popover open={fontOpen} onOpenChange={setFontOpen}>
            ...
            <PopoverContent className="w-48 p-1 z-[120]" align="start">
              {FONT_FAMILIES.map((f) => (
                <Button key={f.label} type="button" variant="ghost" size="sm"
          ...
          {/* B / I / U */}
          <Button type="button" variant="ghost" size="sm" className={iconBtn(isActive("bold"))}
            title={t("personalOutlines.editor.toolbar.bold")}
            onMouseDown={(e) => e.preventDefault()}
            onClick={() => editor.chain().focus().toggleBold().run()}>
            <Bold className="h-4 w-4" />
          </Button>
          <Button type="button" variant="ghost" size="sm" className={iconBtn(isActive("italic"))}
            title={t("personalOutlines.editor.toolbar.italic")}
            onMouseDown={(e) => e.preventDefault()}
            onClick={() => editor.chain().focus().toggleItalic().run()}>
            <Italic className="h-4 w-4" />
          </Button>
          <Button type="button" variant="ghost" size="sm" className={iconBtn(isActive("underline"))}
          ...
          {/* Formatos secundários (Sub/Sup/Code/CodeBlock) agrupados */}
          <Popover open={openG("more")} onOpenChange={setG("more")}>
            <PopoverTrigger asChild>
              <Button type="button" variant="ghost" size="sm"
                className={iconBtn(isActive("subscript") || isActive("superscript") || isActive("code") || isActive("codeBlock"))}
            ...
            {rowPopover(<>
              <Button type="button" variant="ghost" size="sm" className={itemBtn(isActive("subscript"))} onMouseDown={(e) => e.preventDefault()} title={t("personalOutlines.editor.toolbar.subscript")} onClick={() => { editor.chain().focus().toggleSubscript().run(); }}><SubIcon className="h-4 w-4" /></Button>
              <Button type="button" variant="ghost" size="sm" className={itemBtn(isActive("superscript"))} onMouseDown={(e) => e.preventDefault()} title={t("personalOutlines.editor.toolbar.superscript")} onClick={() => { editor.chain().focus().toggleSuperscript().run(); }}><SupIcon className="h-4 w-4" /></Button>
              <Button type="button" variant="ghost" size="sm" className={itemBtn(isActive("code"))} onMouseDown={(e) => e.preventDefault()} title={t("personalOutlines.editor.toolbar.code")} onClick={() => { editor.chain().focus().toggleCode().run(); }}><Code className="h-4 w-4" /></Button>
              <Button type="button" variant="ghost" size="sm" className={itemBtn(isActive("codeBlock"))} onMouseDown={(e) => e.preventDefault()} title={t("personalOutlines.editor.toolbar.codeBlock")} onClick={() => { editor.chain().focus().toggleCodeBlock().run(); closeAll(); }}><Code2 className="h-4 w-4" /></Button>
            </>)}
          </Popover>
          ...
          <Popover open={colorOpen} onOpenChange={setColorOpen}>
            ...
            <PopoverContent className="w-auto p-2 z-[120]" align="start">
              <div className="grid grid-cols-4 gap-1.5">
                {TEXT_COLORS.map((c) => (
                  <button key={c} type="button" className="h-6 w-6 rounded-md border border-border" style={{ backgroundColor: c }} onMouseDown={(e) => e.preventDefault()} onClick={() => { editor.chain().focus().setColor(c).run(); setColorOpen(false); }} aria-label={c} />
                ))}
              </div>
              <Button type="button" variant="ghost" size="sm" className="w-full mt-2 h-7 text-xs" onMouseDown={(e) => e.preventDefault()} onClick={() => { editor.chain().focus().unsetColor().run(); setColorOpen(false); }}>
                <RotateCcw className="h-3 w-3 mr-1" />{t("personalOutlines.editor.toolbar.reset")}
              </Button>
            </PopoverContent>
          </Popover>
          ...
          <Popover open={hlOpen} onOpenChange={setHlOpen}>
            <PopoverTrigger asChild>
              <Button type="button" variant="ghost" size="sm" className={iconBtn(isActive("highlight"))}
            ...
            <PopoverContent className="w-auto p-2 z-[120]" align="start">
              <div className="grid grid-cols-4 gap-1.5">
                {HIGHLIGHT_COLORS.map((c) => (
                  <button key={c} type="button" className="h-6 w-6 rounded-md border border-border" style={{ backgroundColor: c }} onMouseDown={(e) => e.preventDefault()} onClick={() => { editor.chain().focus().toggleHighlight({ color: c }).run(); setHlOpen(false); }} aria-label={c} />
                ))}
              </div>
              <Button type="button" variant="ghost" size="sm" className="w-full mt-2 h-7 text-xs" onMouseDown={(e) => e.preventDefault()} onClick={() => { editor.chain().focus().unsetHighlight().run(); setHlOpen(false); }}>
                <RotateCcw className="h-3 w-3 mr-1" />{t("personalOutlines.editor.toolbar.reset")}
              </Button>
            </PopoverContent>
          </Popover>
      ...
      {row(
        <>
          ...
          <Button type="button" variant="ghost" size="sm" className={iconBtn(isActive("orderedList"))}
            title={t("personalOutlines.editor.toolbar.ordered")}
            onMouseDown={(e) => e.preventDefault()}
            onClick={() => editor.chain().focus().toggleOrderedList().run()}>
            <ListOrdered className="h-4 w-4" />
          </Button>
          <Button type="button" variant="ghost" size="sm" className={iconBtn(isActive("taskList"))}
          ...
          {/* Alinhamento (dropdown com ícone dinâmico) */}
          <Popover open={openG("align")} onOpenChange={setG("align")}>
            <PopoverTrigger asChild>
              <Button type="button" variant="ghost" size="sm" className={iconBtn(false)}
          ...
          {/* Link inline (texto) */}
          <Button type="button" variant="ghost" size="sm" className={iconBtn(isActive("link"))}
          ...
          {/* Inserir (Tabela + edição + HR) */}
          <Popover open={tableOpen} onOpenChange={setTableOpen}>
            <PopoverTrigger asChild>
              <Button type="button" variant="ghost" size="sm" className={iconBtn(isActive("table"))}
            ...
            <PopoverContent className="w-56 p-2 z-[120]" align="start">
              <Button type="button" variant="ghost" size="sm" className="w-full justify-start h-8 text-xs" onMouseDown={(e) => e.preventDefault()}
                onClick={() => { editor.chain().focus().insertTable({ rows: 3, cols: 3, withHeaderRow: true }).run(); setTableOpen(false); }}>
                <TableIcon className="h-3.5 w-3.5 mr-2" />{t("personalOutlines.editor.toolbar.insertTable")}
              </Button>
              <Button type="button" variant="ghost" size="sm" className="w-full justify-start h-8 text-xs" disabled={!isActive("table")} onMouseDown={(e) => e.preventDefault()} onClick={() => editor.chain().focus().addRowAfter().run()}>
                <Plus className="h-3.5 w-3.5 mr-2" />{t("personalOutlines.editor.toolbar.addRow")}
              </Button>
              <Button type="button" variant="ghost" size="sm" className="w-full justify-start h-8 text-xs" disabled={!isActive("table")} onMouseDown={(e) => e.preventDefault()} onClick={() => editor.chain().focus().addColumnAfter().run()}>
                <Plus className="h-3.5 w-3.5 mr-2" />{t("personalOutlines.editor.toolbar.addCol")}
              </Button>
              <Button type="button" variant="ghost" size="sm" className="w-full justify-start h-8 text-xs" disabled={!isActive("table")} onMouseDown={(e) => e.preventDefault()} onClick={() => editor.chain().focus().deleteRow().run()}>
                <Minus className="h-3.5 w-3.5 mr-2" />{t("personalOutlines.editor.toolbar.delRow")}
              </Button>
              <Button type="button" variant="ghost" size="sm" className="w-full justify-start h-8 text-xs" disabled={!isActive("table")} onMouseDown={(e) => e.preventDefault()} onClick={() => editor.chain().focus().deleteColumn().run()}>
                <Minus className="h-3.5 w-3.5 mr-2" />{t("personalOutlines.editor.toolbar.delCol")}
              </Button>
              <Button type="button" variant="ghost" size="sm" className="w-full justify-start h-8 text-xs text-destructive" disabled={!isActive("table")} onMouseDown={(e) => e.preventDefault()} onClick={() => { editor.chain().focus().deleteTable().run(); setTableOpen(false); }}>
                <Trash2 className="h-3.5 w-3.5 mr-2" />{t("personalOutlines.editor.toolbar.delTable")}
              </Button>
              <Button type="button" variant="ghost" size="sm" className="w-full justify-start h-8 text-xs" onMouseDown={(e) => e.preventDefault()} onClick={() => { editor.chain().focus().setHorizontalRule().run(); setTableOpen(false); }}>
                <Divider className="h-3.5 w-3.5 mr-2" />{t("personalOutlines.editor.toolbar.hr")}
              </Button>
            </PopoverContent>
          </Popover>
          ...
          {onToggleFocusMode && (
            <>
              {sep}
              <Button type="button" variant="ghost" size="sm" className={iconBtn(focusMode)}
                title={t("personalOutlines.editor.toolbar.focusMode", { defaultValue: "Modo foco" })}
                aria-pressed={focusMode}
                onMouseDown={(e) => e.preventDefault()}
                onClick={onToggleFocusMode}>
